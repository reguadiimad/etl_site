...
useEffect(() => {
  const fetchMessages = async () => {
    try {
      const response = await axios.get("http://macbook-pro-2.local:8000/api/contact/list/");
      setMessages(response.data);
    } catch (err) {
      setError("Failed to load messges");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="w-full min-h-screen p-6 bg-apple-light/20 rounded-3xl text-apple-dark">
      ...
      >
      Voir chaque Message que vous voulez
      </div>
  ...
  <p>{error}</p>
          ...
          <motion.div
            key={message.id}
            layoutId={`card-${message.id}`}
            onClick={() => setFullInscription(message)}
  ...
  Message: {message.message.split(" ").slice(0, 5).join(" ")}...
</p>
